...
class galera::haproxy ($mysql_user = 'wsrep_sst', $mysql_password = 'password', $haproxy_user = 'haproxy',) {
  exec { "haproxy-monitor":
    ...
    refreshonly => true,
  # unless      => "/usr/bin/mysql -u${mysql_user} -p${mysql_password} -e \"USE mysql; SELECT User FROM user WHERE User =
  # 'haproxy';\"",
  }
}
